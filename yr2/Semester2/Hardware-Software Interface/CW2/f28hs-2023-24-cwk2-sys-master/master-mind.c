...
void waitForButton (uint32_t *gpio, int button) {
  ...
  while(1) {
    if (readButton(gpio, button) == 1) {
      button_press_count++;
      fprintf(stdout, "button presses: %d, button pin: %d, gpio: %d\n", button_press_count, button, gpio);
      delay(500);
      break;
    }
    delay(10);
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  initITimer(3000000);
   fprintf(stdout, "debug 1\n");
    ...
    //while (timeInMicroseconds() - startTime < 3000000) {
    while (timer_running) {
      waitForButton(gpio, BUTTON);
    }
    fprintf(stdout, "debug 3\n");
    
    fprintf(stdout, "3s Passed %d\n", button_press_count);
    
    button_press_count=0;
  
    ////////////////  NEED TO FIX BUTTON AS IT RETUNS VALUES CONSTINTLY    ///////////////
