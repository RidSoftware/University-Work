...
int main (void)
{
  ...
  if ((fd = open ("/dev/mem", O_RDWR | O_SYNC) ) < 0) // omitted O_CLOEXEC mode, as it isnt an argument in this v. of c
 ...
 for (j=0; j<1000; j++)
  {
    ...
    if ((pinACT & 0xFFFFFFC0 /* PI_GPIO_MASK */) == 0)	// bottom 64 pins belong to the Pi	
      {
        // !! important, GPSET1/GPSET1 registers required for pin 47 as 32<47<53 !!
	int off = (theValue == LOW) ? 11 : 8; // ACT/LED 47; register number for GPSET1/GPCLR1
